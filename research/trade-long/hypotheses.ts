...
- Optional exact-date TimesFM forecast via market.timesfm. When available, actively consider a meaningful predictive use within this hypothesis; retain it only when evaluation supports the change. Explain its role and missing-data behavior in strategy.md, or why you chose not to use it. Missing forecasts are neither zero nor positive confirmation; do not carry them forward. Merely declaring the type is not a test, and use is not required in every attempt.
