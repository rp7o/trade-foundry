...
5. **Returns Across Periods**: The evaluator ranks on net profit from a continuous, cost-aware portfolio run starting in 2019. Promotion requires positive portfolio profit in at least 60% of calendar periods, drawdown no greater than 30%, at least 30 trades, activity in at least half the periods, and profit that survives removing the best year. Independent calendar-period backtests provide further stress checks. A long-only strategy can stay in cash during weak periods, but it needs repeatable profit across different markets. Do not optimize for one exceptional year or stock.
